...
% http://tex.stackexchange.com/questions/60360/how-to-use-fraktur-gothic-fonts-in-text-mode
...
%\title{Descriptions and Cardinals Below $\bd^1_5$}
...
%\email{farid.khafizov@utdallas.edu}
...
%Assuming $\ad$, we show that all of the ordinals below $\bd^1_5$ represented
%by descriptions (c.f.\ \cite{J1}) are cardinals. Using this analysis we also get
%a simple representation for the cardinal structure below $\bd^1_5$. 
...
\begin{figure}[h!]
  \centering
    \includegraphics[width=0.9\textwidth]{pics/BasicMeasure}
...
\begin{figure}[h!]
  \centering
    \includegraphics[width=0.9\textwidth]{pics/GenMeasure}
...
%
% to mess with bib style read this
% http://tex.stackexchange.com/questions/168936/bibliography-style-merging-the-alpha-style-with-the-style-of-abbrv-for-author-i
% C:\Program Files\MiKTeX 2.9\bibtex\bst\base
%
%\bibliography{desc}
%\bibliographystyle{plain}
%\bibliographystyle{abbrv}
%\bibliographystyle{C:/Program Files/MiKTeX 2.9/bibtex/bst/base/alpha}
...
\bibitem{Ke}
Alexander~S. Kechris.
\newblock {"Classical Descriptive Set Theory"}, volume 156 of {\em Graduate
...
\bibitem{Ke2}
Alexander~S. Kechris.
\newblock "Homogeneous Trees and the Projective Ordinals".
\newblock In {\em Cabal Seminar 77--79}, volume 839 of {\em Lecture Notes in
  Math.}, pages 33--73. Springer, Berlin, 1978.

\bibitem{KeW}
Alexander~S. Kechris and W.~H. Woodin,
\newblock "Generic codes for uncountable ordinals, partition properties, and elementary embeddings."
\newblock {\em Notes}.

\bibitem{Ma}
D.A. Martin.
\newblock "Borel Determinacy".
\newblock {\em Annals of Math.}, vol 102, p.287--291, 1975.
...
\bibitem{MY}
J. Mycielski.
\newblock "On the axiom of determinateness". I, II,
\newblock {\em Fund. Math.}, vol 53, p.205--224 (1964); vol. 59, p.203--212 (1966).

\bibitem{So}
Robert M. Solovay. 
\newblock "A non-constructible  $\Delta^1_3$ sets of integers."
\newblock {\em Trans. Amer. Math. Soc.}, vol 127, p. 58--75, 1967.

\bibitem{So1}
Robert M. Solovay. 
\newblock "A $\Delta^1_3$ coding of the subsets of $\omega_\omega$."
In {\em Cabal Seminar 76--77}, volume 689, of 
{\em Lecture Notes in   Math.}, pages 133--150. Springer, Berlin, 1978.
...
can be found in \cite{Mo}, \cite{Ke1}, and \cite{So}.
Also, 
...
however, are not important for the current paper, and the reader can 
...
if it is strictly increasing, everywhere discontinuous (that is, 
...
partition of the functions of the correct type, and by assumption there is a c.u.b.\ $C \subseteq
...
also be described as the measure 
...
\cite{Ke1} or \cite{J3}. The second fact is that for any measure 
$\mu$ on $\lambda <\bd^1_3$,
and any $\theta<\bd^1_3$, we have $j_\mu(\theta)<\bd^1_3$. This follows, for example,
from the arguments of \cite{So} or \cite{J3}.
...
$h$ as follows: for $1 \leq j \leq r-1$, we define 
\begin{equation*}
\begin{split}
...
$1 \leq j \leq r$ we define the ``sup'' version $h^s(j)$ by
\begin{equation*}
\begin{split}
h^s(j)(\alpha_1, \dots,
...
\end{equation*}
for $j>1$ and for $j=1$ 
$$h^s(1)(\alpha)=\sup \{ h(\beta_1,\dots,\beta_r)\colon \beta_1<\cdots<\beta_r<\alpha\}.$$
\end{defn}



Note that $h^s(j)$ is 
obtained from $h(j)$ by applying a supremum to the 
...
Note also that for any $h \colon <_r \to \omega_1$ of the correct type,
$h^s(1)$ is the identity function almost everywhere (so, 
we will never have need of $h^s(j)$ for $j=1$).
...
does not 
depend on the value of $m$ or the sequence of measures 
...
$\cD'_m(K_1, \dots, K_t)$. Alternatively, in Lemma~\ref{lemreform} 
we give a purely syntactical reformulation of the ordering. The reader could,
if desired, take  this syntactic reformulation as the definition
of $<$.


The only non-trivial part to check is that 
...
If we had allowed $d=(k;d_1)^s$ as a description, it would have the same interpretation as $d_1$
(since $h^s_k(1)$ is the identity function almost everywhere). 
...
\begin{enumerate}
\item \label{genpos1}
If $i<j$ and $K_i=S_1^{r_i}$, then $h_j$ has range in
a set closed under 
...
In all cases, if $h$ has range in $C'$ 
(the limit points of $C$), then $[h]$ is a limit of $[h']$ where 
$h' \colon \dom(<^{\pi'}) \to C$ is of type $\pi'$ for some permutation $\pi'$ extending $\pi$. 
The reader can consult 
Lemma~4.23 of \cite{J3} for more details.
\end{rem}
...
$\sL^k(d)$ throughout this definition in place of $\sL^k(d;\bar{K})$):
...
is defined (here, and below, if $k=t$
then we regard $k+1$ as being $\infty$), and also  is $> d_l$ in case $l \geq 1$, then
...
Suppose first that $k(d)=k$. If $K_k=W_1^r$, then $d$ is of the form 
...
Consider first the case $k(d')>k=k(d)$. Since $d' < d$, from II.2.\ of
...
$h_k^{(s)}(l'+1)(d_1,\dots,d'_{l'},d_0)$ for some $l' \geq 1$. Since $k(d_0)>k$
in the second case, 
we have from II.2.\ of lemma~\ref{lemreform} that $d_0 < \sL^k(d)$
and so $d' \leq d_0 <\sL^k(d)$. 

Consider next the case $k(d')=k=k(d)$. So now 
...
So, assume $j>0$. 
If $l>j$ or it is the case that $l=j$ and $d$ does not have the symbol $s$,
...
$d_{j-1} =d'_{j-1}<d'_j \leq \sL^{k+1}(d_j)$. 
So, $\sL^k(d)= 
h_k(j+1)(d_1,\dots, d_{j-1},\sL^{k+1}(d_j),d_0)$ from II.1.d.\ of definition~\ref{defl}
(for $j=1$ we have $\sL^k(d)=k_k(2)(\sL^{k+1}(d_1),d_0)$).
...
Also, $d'_{l+1} <d'_0=d_0$ and if $l>0$ then we also have $d'_{l+1} >d_l$
(if $l=0$ then this last inequality is not a requirement in the 
definition of a description).
Thus by induction 
$\sL^{k+1}(d_0)$ is defined and if $l>0$ we also have 
$\sL^{k+1}(d_0)\geq d'_{l+1}> d_l$.
So, from 
...
Then $\sL^k(d)$ is of the form $h_k^{(s)}(a+1)(d_1,\dots,d_l,f_{l+1},\dots, f_a,d_0)$
for some $a \geq l$. 
...
(again, if $k=t$ then by $k+1$ we mean $\infty$).
...
h_k(1)(\sL^{k+1}(d))$. We then have from cases III.2.a,c,d of lemma~\ref{lemreform}
that $d' \leq \sL^k(d)$. 
...
It then follows from I.2.\ of lemma~\ref{lemreform} that $d <d'$. 


Suppose next (still proving property~(\ref{lemonsup3})) that $k<n$. 
If $d \in \cD_m(K_1,\dots,K_{n-1})$, then from property~(\ref{lemonsup2})
we have $d'=d$ and we are done. 
Otherwise, let $i\geq 0$ be least so that $f_i \notin 
\cD_m(K_1,\dots,K_{n-1})$. From property~(\ref{lemonsup1}) we have that 
$f'_i \neq f_i$. From property~(\ref{lemonsup2}), $i$
is also least such that $f'_i\neq f_i$. If $i=0$, then 
$d'=f'_0$. Since $f'_0 >f_0$ in this case, we have that 
$d'>d$ from II.2.a of lemma~\ref{lemreform}. Suppose
then that $i>0$. Since $f'_i \in \cD_m(K_1,\dots,K_{n-1})$
we have $f'_i\neq f_i$, and therefore $f'_i>f_i$. It then 
follows again from II.2.a of lemma~\ref{lemreform} that 
$d'>d$.
...
By induction, $q'_l \leq d_l$  and so 
from 
III.2.b and III.2.d of lemma~\ref{lemreform}
...
From 
III.2.c of lemma~\ref{lemreform}
...
III.2.c of lemma~\ref{lemreform} again.
\end{proof}
...
\begin{example}
We use the same measure sequence $\bar{K}$ and description $d^{(f_4)}$
...
\[
[g]_{W_1^4}  \mapsto 
[\bar{\beta} \mapsto h_1(3)(\alpha_{3,1},g(\bar{\beta}), h_2(2)(h_4(1)(\beta_2),\beta_3))]_{W_1^4}
\]
is cofinal from $(h_2(2)(h_4(1)(\cdot_2), \cdot_3); \bar{h})$ to $(d;\bar{h})$.
...
definition~\ref{defl} of the $\sL$ operation (using here that $j \leq t+1$).
...
it also therefore appears in the functional representation
of $\sL^j(\bar{\bar{p}})$. 
...
\begin{proof}
Assume $\cof(\bar{p})=\omega$. We proceed by reverse induction on $j$. 
...
show that $(g;d;W_3^m;\bar{S})$ is not a cardinal. This was, in fact,
...
The reader of this paper  does not need to be familiar with the 
...
h^s_i(j)$, $\alpha_{i,j}$ when  referring
to the measures in
...
exist then the definition stops at $v'(i)$. 
\end{defn}
...
Note that in the inductive definition of $\ods{q}{\bar{d}}$ where 
$q=g(d_1,\dots,d_\ell, d_0)$, we place $\ods{d_0}{\bar{d}}$
before the terms in $\ods{d_i}{\bar{d}}$ for $i>0$. However, in placing the superscripts
on the terms of the $\ods{q}{\bar{d}}$, we consider  $d_0$ placed
after the $d_i$ terms. 
...
\begin{example}
For $(\bar{S},\bar{K})=(S_1,S_2,K_3,K_4,K_5)$ with, say, all measures equal to $S_1^4$, and $q$ the description
$$q=h_1(3)(h_2(3)(k_3(\cdot_2), k_4(\cdot_2), \cdot_3),
\ h_2(3)(k_3(\cdot_2),  k_5(\cdot_3),\cdot_4),
\ h_2(3)(k_3(\cdot_2),  k_5(\cdot_3), \cdot_5)),$$
\noindent
we have
\begin{equation*}
\noindent
\begin{split}
\od{q}{\bar d} =& (k_3(\cdot_2), k_5(\cdot_3)),\text{ and }\\ 
\ods{q}{\bar d}=&(k^3_3(\cdot_2), k^2_5(\cdot_3), k^1_3(\cdot_2),
k^1_4(\cdot_2), k^2_3(\cdot_2), k^1_5(\cdot_3)).
\end{split}
\end{equation*}
\end{example}
...
embedding argument (lemma \ref{mainlem}) that 
...
$q \in \cD_m(\bar{S})$ with $q \leq d$ and $q$ satisfying condition~D, which is clearly finite.
...
with $p \leq d$ is in one of these blocks. 
...
Let $\bl{q_1}{\bar{d}},\dots,\bl{q_n}{\bar{d}}$ be all the blocks of 
$\bar{d}$ where $q_1=d> q_2=p > q_3 > \dots > q_n$ and ${q_i} \in \cD_m(\bar{S})$.
...
$\bar{p}$--blocks. Let $s^i_1>s^i_2>\cdots>s^i_{e_i}$ enumerate all the 
$s \in \cD_m(\bar{S},S^*)$ with $q_{i+1}<s \leq q_i$. 
So, $s^i_1=q_i> s^i_2 > \cdots >s^i_{e_i}>q_{i+1}$. Thus, the $\bar{d}$--block
...
corresponding 
block $\bl{q_i}{\bar{d}}$. We let $e$ denote $e_i$. 
Again let $s^i_1=q_i>s^i_2>\cdots>s^i_e>q_{i+1}$ as above so $\bl{q_i}{\bar{d}}$
splits into the $\bar{p}$--blocks $\bl{s^i_j}{\bar{p}}$ for $1 \leq j \leq e$. 
...
$\od{q'}{\bar{d}}$ begins with a $k_*$ term as otherwise 
...
still less than the right-hand side. 
...
on $\kappa$, the ultrapower $j_\nu(\kappa)$ is a cardinal.
\end{thm}
...
\begin{enumerate}
\item \label{stremb1}
$\forall^\ast_\mu \theta \ H(\theta)=([\phi_1(\theta)]_{M_1}, \dots,
[\phi_l(\theta)]_{M_l})$, where $\phi_i(\theta)\colon   D_i \to E_i$ is order-preserving.
\item  \label{stremb2}
For all $A_i \subseteq E_i$, $1 \leq i \leq n$,  of $N_i$
measure 1, $\forall^\ast_\mu \theta$
$\forall i \ \forall^\ast_{M_i} \alpha \in D_i$ $(\phi_i(\theta))(\alpha) \in A_i$.
\end{enumerate}
\end{defn}


In writing properties (\ref{stremb1}) and (\ref{stremb2})
of definition~\ref{stremb}, we will usually just write 
$\phi_i$ instead of $\phi_i(\theta)$, the dependence on $\theta$
being understood. 
...
witness the strong embeddability of $(D_i,M_i)$ into $(E_i,N_i)$,
...
Given the ordering $D=D_1 \oplus \dots \oplus D_n$ (of order-type $<\bd^1_3$) 
...
$H(\bar{\alpha})$ be the $n$-tuple of ordinals represented by the function
(which we also call $H(\bar{\alpha})$)
...
we define 
$G([f],\alpha)=F([f])$. 
Note that $[f]$ actually refers to the tuple of ordinals $(\dots,[f_i]_{M_i},\dots)$;
we make this notational identification below as well.
This is easily an embedding and maps 
...
Suppose now $M=M_1\times M_2 \times \cdots \times M_k$ is a product of measures
where each $M_j$ if of the form $W^1_1$ or $S_1^r$. 
We define the arity function $r$ by letting 
$M_j=S_1^{r(j)}$, if $M_j$ is of the form $S_1^r$,
and letting $r(j)=0$ if $M_j=W_1^1$.
Let $\pi=(p_1, \dots, p_k)$ be a permutation of $k$.
Let $D$ be the $M$
measure one set of $(\beta_1, \dots, \beta_k)$ 
which are in general position, that is, 
if $i <j$ then if $r(i)=r(j)=0$ we have $\beta_i<\beta_j$, 
and if $r(i), r(j)>0$ then we have $\beta_i(1)<\beta_j(1)$ 
(recall definition~\ref{definvariants}
and the comments immediately after).


Let $\pi^*=(q_1,\dots,q_l)=(p_{s_1},\dots,p_{s_l})$, where $l \leq k$, be 
the canonical increasing subsequence of $\pi$. That is, 
$q_1=p_1$ (i.e., $s_1=1$), and $q_{i+1}=p_{s_{i+1}}$ where 
$s_{i+1}$ is the least integer greater than $s_i$ such that 
$p_{s_{i+1}} >p_{s_i}$. Thus, $q_1<q_2< \cdots<q_l=k$. 
Let $N$ be the product measure $\prod_{i=1}^l M_{q_i}$
where the terms in the product are written in the same order as in $M$,
that is $M_{p_{s_a}}$ is written before $M_{p_{s_b}}$
iff $p_{s_a}<p_{s_b}$ iff $a<b$. Let $E$ be lexicographic ordering
on tuples $(\alpha_1, \dots, \alpha_l)$ with $\alpha_i < \omega_{r(q_i)+1}$.
...
\begin{proof}
Let $\mu=(W_1^1)^k \times M_1 \times \dots \times M_{q_1-1} \times
\prod_{j=q_1}^k M_j^+$, where $(W^1_1)^+=S^1_1$, and
$(S^r_1)^+=S^{r+1}_1$. 
Fix $\bar{\theta}=(\eta_1,\dots,\eta_k,\theta_1, \dots, \theta_k) \in \dom(\mu)$,
so $\eta_1,\dots,\eta_k<\omega_1$. 
Let $h_j \colon \dom(<_{r(j)+1})
\to \omega_1$ represent $\theta_j$ if $r(j)>0$.
...
If $r(j)=0$, set $\beta_j= h_j(\alpha_{i'})$,
where $i'\leq i$ is largest so that $r(q_{i'})=0$
if such an $i'$ exists, and otherwise set 
$\beta_j= h_j(\eta_{j})$.
If $r(j)>0$ and $r(q_{i'})=0$ for all $i'\leq i$, set $\beta_j =[g_j]$, where
$g_j(\gamma_1, \dots, \gamma_{r(j)})= h_j(\alpha_i,\gamma_1,
\dots, \gamma_{r(j)})$. Otherwise, there is a largest $i'\leq i$
with $r(q_{i'})>0$, and we set
$\beta_j=[g_j]$, where $g_j$ is defined as follows. 
If $j=q_i$ (so $i'=i$), then 
$$g_j(\gamma_1, \dots, \gamma_{r(q_i)})= 
h_j(\gamma_1, \dots, \gamma_{r(q_i)-1},
f_i(\gamma_1,\dots,\gamma_{r(q_i)}), 
f_{i}(1)(\gamma_{r(q_i)})),$$ 
where $[f_i]=\alpha_i$. In this formula, if $r(q_i)=1$,
then the term $f_i(\gamma_1,\dots,\gamma_{r(q_i)})$ is omitted,
and the right-hand side is $h_j(\gamma_1, f_i(\gamma_1))$.
If $j>q_i$ then we set 
$$g_j(\gamma_1, \dots, \gamma_{r(j)})= 
h_j(\gamma_1, \dots, \gamma_{r(j)},
f_{i'}(1)(\gamma_{r(j)})).$$ 
...
if $\pi'$ is any subsequence of the canonical increasing subsequence $\pi^*$
...
$\bl{q}{\bar{d}}$ which is non-trivial, that is, with $\dep{\bl{q}{\bar{d}}}>1$.
Then there is a $\bar{p}=(p;\bar{S},\bar{K}) \in 
\bl{q}{\bar{d}}$  with $\lv{p}{\bar{d}}=
\dep{\bl{q}{\bar{d}}}$ where $p$ is of the form $p=\gamma_{a,b}$,
$p=k_a(\v{r})$. or 
$p=h_k^{(s)}(l+1)(f_1,\dots,f_l,f_0)$. In the last case, 
there is a $\bar{p}' \in \bl{q}{\bar{d}}$
with $\lv{p'}{\bar{d}}=
\dep{\bl{q}{\bar{d}}}$ and such that $p'$ is of the form 
$$p'=h_k^{(s)}(r)(g_1,\dots,g_{r-1},g_0)$$ where $S_k=S_1^{r}$
(that is, $p'$ has maximal possible length).
\end{prop}
...
\begin{proof}
Say $d \in \cD_m(\bar{S})$ and recall that $q \in \cD_m(\bar{S})$ as well. 
Note first that $\cof(q)>\omega$. For if $\cof(q)=\omega$,  
then the block $\bl{q}{\bar{d}}$ would be trivial 
(i.e., consist only of $q$)
and so $\dep{\bl{q}{\bar{d}}}=1$. For suppose $\bl{q}{\bar{d}}$ is not trivial
and let $\bar{p}=(p;\bar{S},\bar{K})$
for some measure sequence $\bar{K}$
be such that $\sup_{\bar{K}}(p)=q \ne p$. 
Note that $\sL(q;\bar{S},\bar{K})$
is defined and $p \leq \sL(q;\bar{S},\bar{K})$ by the maximality of 
$\sL(q;\bar{S},\bar{K})$ among those descriptions in 
$\cD_m(\bar{S},\bar{K})$ which are less than $q$.
By proposition~\ref{prop1b} we have 
that $\sL(q; \bar{S},\bar{K}) \in \cD_m(\bar{S})$. Thus, 
$\sL(q; \bar{S},\bar{K})=\sL(q;\bar{S})$. But then by  
proposition~\ref{suppropb} we have $\sup_{\bar{K}} (p) \leq 
\sL(q;\bar{S})$, a contradiction. 
...
Suppose first that $q$ has functional representation
$q=h_k(l+1)(f_1, \dots,f_l,f_0)$, and $S_k=S^{r}_1$.
We must have
$l<r-1$, as otherwise $\cof(q)=\omega$ from (\ref{lemoncof3})
of lemma~\ref{lemoncof}. We can also see this directly as follows. 
Since $\bl{q}{\bar{d}}\neq \emptyset$, there is a $\bar{p}=
(p;\bar{S},\bar{K})$ with $\sup_{\bar{K}}(p)=q$. 
...
$\cof(q)=\cof(f_0)$, so $\cof(f_0)>\omega$. 
Let now $\bar{p}=(p; \bar{S},\bar{K}) \in \bl{q_i}{\bar d}$ have
maximum possible level, say $p$ is of the form 
$$p = h_{k'}^{(s)}(l'+1)(f'_1,\dots,f'_{l'},f'_0).$$
Since replacing $h_{k'}^s(l'+1)$ 
by $h_{k'}(l'+1)$ in the functional representation for $p$ 
does not change either the level or the block, we may
assume without loss of generality that $p=h_{k'}(l'+1)(f'_1,\dots,f_{l'}\dots,f'_0)$.
We must have $k'\leq k$ as otherwise $k=k(q)=k(\sup_{\bar K}(p))
\geq k'>k$ (using here (\ref{lemonsup2b}) of Lemma~\ref{supprop}).
...
Consider now the pre-description (where $S_{k'}=S_1^{r'}$)
$$p'=h_{k'}(r')(f'_1,\dots,f'_{l'},g_{l'+1},\dots,g_{r-1},f' _0)$$
where for $l'+1 \leq j \leq r-1$ we let 
$g_j=\sL^{k+1}(f_0; \bar{S},\bar{K},M_1,\dots, M_{j-l'})$ where each $M_a=M$. 


We show that $p'$ is actually a description. First we show that $g_j<f'_0$ for each $j$. 
If $k'=k$, the $p$ is of the form 
$p=h_k(l'+1)(f_1,\dots,f_l,\dots,f'_{l'},f_0)$, in particular $f'_0=f_0$
(this follows from the definition of $\sup_{\bar{K}}(p)$ being $q$). 
So, $g_j<f_0 =f'_0$ in this case. Suppose next that $k'<k$. 
We claim that $f_0\leq f'_0$ which suffices. If $f'_0 < f_0$,
then $\sup_{\bar K}(f'_0) \leq f_0<q$. However, 
$\sup_{\bar K} (p) \leq \sup_{\bar K} (f'_0) <q$, a contradiction 
to $\sup_{\bar K} (p)=q$.  Next we show that $g_j>f_{l'}$.
